...
//
// This endpoint re-syncs the already-seeded `step8` row to match the
// current db/seed-data.json content, for this one content correction.
// It is meant to be hit once via GET and then removed in a follow-up
// commit - it should not stay in the codebase long-term.
const { getPool, ensureReady } = require('./_db');
const seedData = require('../db/seed-data.json');

const RESYNC_TOKEN = 'oijhWLYUmY7iRq9Ba0F26L1wUOgm5PXA';
...
module.exports = async function handler(req, res) {
  ...
  const section = seedData.find((s) => s.slug === 'step8');
  if (!section) {
    res.status(500).json({ error: 'step8 not found in seed data' });
    return;
  }
