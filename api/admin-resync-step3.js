...
//
// This endpoint re-syncs the already-seeded `step3` row to match the
// current db/seed-data.json content, to push out the new "Turn off
// notifications for this repo" subsection. It is meant to be hit once via
// GET and then removed in a follow-up commit - it should not stay in the
// codebase long-term.
const { getPool, ensureReady } = require('./_db');
const seedData = require('../db/seed-data.json');

const RESYNC_TOKEN = '3NuH92ifmOb1q3xqR42uJr9vqXVjpEaf';
...
module.exports = async function handler(req, res) {
  ...
  const section = seedData.find((s) => s.slug === 'step3');
  if (!section) {
    res.status(500).json({ error: 'step3 not found in seed data' });
    return;
  }
